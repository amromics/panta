...
def qscore_parallel(test_list, reference_list):
    with multiprocessing.Pool(processes=threads) as pool:
        pool.starmap(run_qscore, zip(test_list, reference_list))
...
def run_iqtree_alisim():
    ...
    random_tree = '-t RANDOM{yh/100} -rlen 0.001 0.05 0.999 '
    seq_length = '--length 1000 '
...
if __name__ == "__main__":
    ...
    global timing_log
    timing_log = '/home/noideatt/TA/simulate/time.log'
    ...
    for ref_msa in ref_list:
        calucate_average_identity(ref_msa)
